...
import applauseFile2 from "./assets/audio/Clapping.mp3";
...
const getVolume = (config, count = 0) => {
  ...
  switch (config["scale"]) {
    case "quad":
      return clamp(Math.sqrt(x), 0, 0.5);
    case "exp":
      const base = Math.E;
      const y = (Math.pow(base, x) - 1) / (base - 1)
      return clamp(y, 0, 0.5);
    case "linear":
    default:
      return clamp(x, 0, 0.5);
  }
}
const Client = () => {
...
const Player = ({ file, name, config, volume: volumeProp = 50, loop = false }) => {
  ...
  useEffect(() => {
    ...
    const vol = getVolume(config, parseInt(volume, 10))/2.25;
...
const processCheerCounts = (cheerCounts) => {
  ...
  for (let reaction in cheerCounts) {
    ...
    for (let variation in cheerCollection[reaction]) {
      cheerVolumes[reaction][variation] = cheerCounts[reaction];//getVolume(cheerCollection[reaction][variation], cheerCounts[reaction]);
    }
  }
  return cheerVolumes;
}
